...
| Status | Meaning |
|---|---|
| ✅ covered | Automated, copy in a separate failure domain (offsite or at least another machine), restore procedure written |
| 🟡 partial | Automated but missing one of: offsite copy, restore test, monitoring, sane retention |
...
|---|---|---|---|---|---|---|---|---|---|---|
| Immich database (assets, faces, embeddings) | immich | `local-path` on `k8s-worker-1` (Proxmox VM, single NVMe) | CNPG Postgres 17 + vectorchord | barman-cloud: weekly base + continuous WAL to Scaleway S3 | Immich built-in `pg_dumpall`, daily, on NAS | S3 yes, dump no | 30d (S3), 14 dumps | 🟡 partial | never | [apps/immich.md](apps/immich.md) |
...
| Spliit database (expenses) | spliit | `nfs-client` (brassberry-25 USB disk, the default SC) | CNPG Postgres 18 | barman-cloud: weekly base + WAL to Scaleway S3 | none | yes | 30d | 🟡 partial | never | [apps/spliit.md](apps/spliit.md) |
...
- Restore drill done: fill the "Last restore test" column and the drill log in
  [strategy.md](strategy.md#restore-drill-log).
