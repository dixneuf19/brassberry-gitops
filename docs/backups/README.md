...
| ⚪ accepted risk | No backup on purpose, the reason is written down. Includes data that expires by design (metrics, logs) |
| ♻️ rebuildable | Derived data (cache, index, thumbnails), regenerated by the app from its primary data, no backup wanted |
...
| Prometheus TSDB | monitoring | `local-path` on brassberry-27 (NTFS USB disk) | TSDB | none | none | no | 30d | ⚪ accepted risk (history is not rebuildable, it expires after 30d anyway) | n/a | [apps/monitoring.md](apps/monitoring.md) |
