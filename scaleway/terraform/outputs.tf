...
# --- htpasswd secrets (manual passwords, ESO renders bcrypt) ---

output "secret_id_basic_auth" {
  description = "Secret ID for basic-auth (JSON: username, password) -- ESO renders htpasswd"
  value       = scaleway_secret.basic_auth.id
}

output "secret_id_files_auth" {
  description = "Secret ID for files-auth (JSON: username, password) -- ESO renders htpasswd"
  value       = scaleway_secret.files_auth.id
}

output "secret_id_yoshi_basic_auth" {
  description = "Secret ID for yoshi-basic-auth (JSON: username, password) -- ESO renders htpasswd"
  value       = scaleway_secret.yoshi_basic_auth.id
}
