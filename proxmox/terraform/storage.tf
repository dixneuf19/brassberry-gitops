...
resource "proxmox_storage_zfspool" "fastpool" {
  id       = "fastpool"
...
resource "proxmox_storage_directory" "tank_backups" {
  id      = "tank-backups"
  path    = "/tank/backups"
  nodes   = [var.proxmox_node]
  content = ["backup"]
...
resource "proxmox_storage_directory" "tank_iso" {
  id      = "tank-iso"
  path    = "/tank/iso"
  nodes   = [var.proxmox_node]
  content = ["iso", "import"]
...
resource "proxmox_storage_directory" "tank_templates" {
  id      = "tank-templates"
  path    = "/tank/templates"
  nodes   = [var.proxmox_node]
  content = ["vztmpl"]
